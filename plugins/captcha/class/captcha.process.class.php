...
class Traum_captcha
{
    public function Chemical()
    {
        $db = new DB;
        $id = rand(1, 10186);
        $results = $db->get_row("SELECT * FROM `captcha_vcode` WHERE `id` =$id");
    ...
    public function History()
    {
        $db = new DB;
        $id = rand(1, 30925);
        $results1 = $db->get_row("SELECT * FROM `captcha_event` WHERE `id` =$id");
        $type = $results1['y'];
        $year = $results1['d'];
        $riqi = $results1['i'];
        $info = $results1['p'];
        ...
        $box = new Box($im);
        $box->setFontFace(DIR . '/assets/fonts/fz.otf');
    ...
    {
        $db = new DB;
        $image = imagecreatefrompng(DIR . "/assets/img/bg.png");
        $black = imagecolorallocate($image, 0, 0, 0);
        $id = rand(1, 22523);
        $size = 22;
        $font = DIR . '/assets/fonts/fz.otf';
        $text = "1";
        $results1 = $db->get_row("SELECT * FROM `captcha_matrix` WHERE `id` =$id", ARRAY_A);
        $m1 = $results1['m1'];
        $m2 = $results1['m2'];
